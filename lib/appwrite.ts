...
export async function login() {
  try {
    ...
    if (!response) throw new Error("Create OAuth2 token failed - 1 !response");
    ...
    if (browserResult.type !== "success")
      throw new Error(
        "Create OAuth2 token failed - 2 browserResult.type !== success"
      );
    ...
    if (!secret || !userId)
      throw new Error("Create OAuth2 token failed - 3 !secret || !userId");
